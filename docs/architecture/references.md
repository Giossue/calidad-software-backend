# Referencias técnicas oficiales

Estas fuentes sustentan el baseline. Comprueba la versión instalada antes de
aplicar una API y registra desviaciones importantes en un ADR.

## Laravel 13

- [Controllers](https://laravel.com/docs/13.x/controllers)
- [Validation and Form Requests](https://laravel.com/docs/13.x/validation)
- [Authorization and Policies](https://laravel.com/docs/13.x/authorization)
- [Authentication](https://laravel.com/docs/13.x/authentication)
- [Fortify](https://laravel.com/docs/13.x/fortify)
- [Eloquent relationships](https://laravel.com/docs/13.x/eloquent-relationships)
- [Database and transactions](https://laravel.com/docs/13.x/database)
- [Migrations](https://laravel.com/docs/13.x/migrations)
- [HTTP tests](https://laravel.com/docs/13.x/http-tests)

## Inertia y React

- [Inertia forms](https://inertiajs.com/docs/v3/the-basics/forms)
- [Inertia shared data](https://inertiajs.com/docs/v3/data-props/shared-data)
- [Inertia partial reloads](https://inertiajs.com/docs/v3/data-props/partial-reloads)
- [Inertia testing](https://inertiajs.com/docs/v3/advanced/testing)
- [React: Thinking in React](https://react.dev/learn/thinking-in-react)
- [React: Keeping Components Pure](https://react.dev/learn/keeping-components-pure)

## PostgreSQL

- [Constraints](https://www.postgresql.org/docs/current/ddl-constraints.html)
- [Indexes](https://www.postgresql.org/docs/current/indexes.html)
- [Transaction isolation](https://www.postgresql.org/docs/current/transaction-iso.html)
